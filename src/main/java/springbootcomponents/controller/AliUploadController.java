...
public class AliUploadController {
    ...
    private boolean aliHasInitialized=false;
    ...
    @RequestMapping("/do.ali.uploader")
    @ResponseBody
    public Map<String, Object> ali_upload(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if((!headerHascontype)||(!request.getHeader("content-type").contains("multipart/form-data"))){
            ...
            if(!aliHasInitialized){
                AliyunPost.InitUrl();
                aliHasInitialized=true;
            }
